    ...
    displayItems() {
        ...
        $$('.inventorySlot').forEach((slot, index) => {
            ...
            if (GAME.activePlayer().items[index] !== undefined) {
                ...
                slot.style.backgroundImage = `url(./media/items/${GAME.activePlayer().items[index].img})`
    ...
    displayEquipment() {
        ...
        for (let item in GAME.activePlayer().equipment) {
            ...
            if ( GAME.activePlayer().equipment[item] !== undefined) {
                ...
                $(`.${GAME.activePlayer().equipment[item].slot}`).style.backgroundImage = `url(./media/items/${GAME.activePlayer().equipment[item].img})`;
                $(`.${GAME.activePlayer().equipment[item].slot}`).dataset.bonus = GAME.activePlayer().equipment[item].bonus;
