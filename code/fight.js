...
const FIGHT = {
    ...
    createScene() {
        ...
        $(".fightDisplay").innerHTML +=
            this.fighterConstructor(GAME.activePlayer()) +
            this.fightStatsConstructor() +
            this.fighterConstructor(this.activeNPC);
                ...
                <div class="healthBar">
                    <div class="barContainer">
                        <div class="bar" style="width:${
                            (100 / fighter.maxHealth) * fighter.health
                        }%" ></div>
                    </div>
                 </div>
    ...
    displayChoices(playerAction, npcAction) {
        $(".playerFightAction").innerHTML = `<div class="${
            playerAction.action === "attack" ? "attack" : "defence"
        }">
            ${
                playerAction.action === "attack"
                    ? playerAction.die + GAME.activePlayer().attack
                    : playerAction.die + GAME.activePlayer().defence
            }
        </div>`;

        $(".npcFightAction").innerHTML = `<div class="${
            npcAction.action === "attack" ? "attack" : "defence"
        }">
            ${
                npcAction.action === "attack"
                    ? npcAction.die + this.activeNPC.attack
                    : npcAction.die + this.activeNPC.defence
            }
        </div>`;
    },
    ...
    updateNpc() {
        ...
        $(".npcFighter .bar").style.width =
            (100 / this.activeNPC.maxHealth) * this.activeNPC.health + "%";
    },
    ...
    updateVisuals(winner) {
        if (winner.name === "none") {
            ...
            GAME.activePlayer().health =
                GAME.activePlayer().health - winner.hit < 0
                    ? 0
